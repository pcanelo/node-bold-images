...
### ⚠️ ESTADO DE SOPORTE (Actualizado: 13 Mayo 2026)

| Versión | Estado Real | Nivel de Riesgo | Acción Requerida |
| :--- | :--- | :--- | :--- |
| **Node 14, 16** | **End-of-Life (EOL)** | 🚨 **CRÍTICO** | Migración inmediata. No reciben parches de seguridad. |
| **Node 18** | **End-of-Life (EOL)** | 🚨 **CRÍTICO** | EOL desde Abril 2025. Migración inmediata. |
| **Node 20** | **Maintenance LTS** | ⚠ **MEDIO** | EOL Abril 2026. Planificar migración a Node 22 o 24. |
| **Node 22** | **Active LTS** | ✅ **MÍNIMO** | Versión estándar recomendada. |
| **Node 24** | **Active LTS** | ✅ **MÍNIMO** | Versión estándar recomendada. |

## Prácticas de Seguridad Implementadas

### 1. Ejecución Non-Root Obligatoria
Todas las imágenes runtime están configuradas para ejecutarse bajo el usuario `nodeuser` con UID/GID 10001. Este usuario carece de privilegios administrativos y tiene asignado `/bin/false` como shell, lo que impide accesos interactivos no autorizados y mitiga el impacto en caso de una ejecución de código arbitrario.

### 2. Validación de Integridad Mediante Digests
Para garantizar builds determinísticos y prevenir ataques de sustitución de imágenes, el uso de tags flotantes (ej. `debian:bookworm-slim`) se limita a imágenes de desarrollo y etapas builder. En las etapas finales runtime (producción), es obligatorio referenciar las imágenes base mediante su Digest SHA256 inmutable. Adicionalmente, se valida el hash de los binarios de Node.js durante la construcción de la imagen base para asegurar que no han sido alterados.
...
- **Imágenes dev:** Contienen compiladores (`g++`, `make`) y herramientas de depuración necesarias únicamente para el build.
- **Imágenes runtime:** Se excluyen deliberadamente todas las herramientas de desarrollo, utilidades de descarga (`curl`) y gestores de paquetes innecesarios, reduciendo drásticamente los vectores de ataque.
...
```bash
trivy image registry.empresa.com/base/node22-runtime:latest
```
...
2. Actualizaciones de paquetes del sistema operativo (rotación de digests en runtime).
3. Mejoras en las prácticas de seguridad.

Las actualizaciones se comunican a través de cambios de versión en los tags de las imágenes y se documentan en `CHANGELOG.md`.

## Recomendaciones para Equipos Consumidores

1. **Utiliza versiones Active LTS:** Prefiere Node.js 22 o 24 para todos los proyectos nuevos y migraciones.
...
5. **No uses dev en producción:** Las imágenes `dev` están prohibidas en entornos productivos.

## Documentos Relacionados

- [`política_corporativa_imagenes_base_docker_node.js.md`](política_corporativa_imagenes_base_docker_node.js.md) — Política corporativa completa.
- [`política_rotación_de_digests_docker_base.md`](política_rotación_de_digests_docker_base.md) — Procedimiento de rotación de digests.
